...
def evaluate_model_on_dataset(model, tokenizer, dataset, batch_size=16):
    model.eval()
    preds = []
    ...
    for batch in tqdm(data_iterator, total=len(loader)):
        with torch.no_grad():
            ...
            inputs = tokenizer(samples, return_tensors="pt", padding=True, truncation=True, return_attention_mask=True, max_length=250)
...
def inference(args):
    ...
    test_dataset = format_dataset(args.test_data_file)
